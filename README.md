...
- 🚢 Shipped services that scale and survive earthquakes.
- 🏗️ Built HealthMetrics system, looking for the next big impact!
- ⚙️ Low-friction-high-output engineering workflows and platform.
- 🧩 Turning ambiguity > plans > shipped products.
- 🐙 Organised budgets, teams, events, and more...
- 🤖 Exploring AI-assisted coding, agent orchestration, AI-driven systems~

## 🛠️ Toolbox

**Primary Depth**
`C#` `.NET` `JavaScript` `TypeScript` `Azure` `SQL Server`

**Worked with**
`Python` `HTML/CSS` `PowerShell` `C` `C++` `Java` `PHP` `SQL`
`Assembly` `Docker` `Redis` `MongoDB` `MySQL` `SQLite` `Snowflake`
`GCP` `Cloud Functions` `Kubernetes` `Terraform` `Digital Ocean`
